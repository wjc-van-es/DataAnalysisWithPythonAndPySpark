...
try:
    spark.sql(
        ...
        select count(*) 
        from {table_name} 
        where radioactive is not null 
        '''
    ).show()
except AnalysisException as e:
    print(e)

print(f"number of radioactive elements = {elements.where('radioactive is not null').select(F.col('AtomicNumber')).count()}")
print(f"number of radioactive elements = {elements.where(F.col('Radioactive').isNotNull()).count()}")
elements.where(F.col("Radioactive").isNotNull()).groupby().count().show()

try:
    spark.sql(
        f'''
        select AtomicNumber, Element, Symbol, Period, Group, Radioactive
        from {table_name} 
        where radioactive is not null 
        '''
    ).show(37)
